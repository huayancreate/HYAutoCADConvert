...
namespace Search
{
    public class MysqlOperate
    {
        ...
        public bool UserLogin(string userName, string userPass)
        {
            var strSql = "select * from users where username='" + userName + "' and userpass='" + userPass + "'";
            DataSet ds = MysqlDBUtil.Query(strSql);
            var count = 0;
            if (ds.Tables.Count > 0)
            {
        ...
        {
            var strSql = "update users set userpass='" + newPass + "' where userpass='" + oldPass + "'";
            if (MysqlDBUtil.ExecuteSql(strSql) > 0)
            {
                return true;
            }
            return false;
