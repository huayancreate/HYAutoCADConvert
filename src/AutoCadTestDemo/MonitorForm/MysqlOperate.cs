...
namespace Monitor
{
    public class MysqlOperate
    {
        ...
        public int DrwingsCount(string status)
        {
            var strSql = "select * from history";
            if (!string.IsNullOrEmpty(status))
            {
                strSql += " where filestatus = " + status;
            }
            return dbUtil.GetRowsNum(strSql);
        }

        public MysqlOperate(string connection)
        {
